function [W1, b1, W2, b2, W3, b3, W4, b4, mseValues, accValues] = backProp4Layer(trainInputs, trainTargets, learningRate, iterations, testInputs, testTargets, testLabels)
...
W3 = zeros(hiddenLayer3, hiddenLayer2);
b3 = rand(hiddenLayer3, 1);

for m = 1:hiddenLayer3
...
% ---- Layer 4 ----- %
%[w2rows w2cols] = size(W2);
W4 = zeros(outputRows, hiddenLayer3);
b4 = rand(outputRows, 1);

for m = 1:outputRows
    for n = 1:hiddenLayer3
        x = randi(2);
        if(x == 2)
            x = -1;
        end
...
while( mseIter > 0.005 && iters < iterations + 1)
    ...
    for passes = 1:trainCols 
        ...
        a4 = logSigmoid((W4 * a3) + b4);

        error = target - a4; % error vector 
        mseIter = mseIter + mse(target, a4); % add this pair's MSE
        
        %------ Now Calculate Sensitivities and Backpropagate ------%
        
        F2n4 = zeros(outputRows, outputRows);
        for i = 1:outputRows
            for j = 1:outputRows
                if i == j
                    F2n4(i,j) = (1 - a4(i,1))*(a4(i,1));
                end
            end
        end
        
        F2n3 = zeros(hiddenLayer3, hiddenLayer3);
        for i = 1:outputRows
            for j = 1:outputRows
                if i == j
        ...
        s4 = -2 * F2n4 * error;  % creates vector sized outputRows x 1
        
        s3 = F2n3 * W4' * s4;  % creates vector sized outputRows x 1
    ...
    x = mod(iters,100);
    if(x == 0)
        accValues(iters/100,1) = determineAccuracy((validationSetTest3Lay(testInputs, testTargets, W1, b1, W2, b2, W3, b3)), testLabels)
    end
end
