function [W1, b1, W2, b2, W3, b3, mseValues, accValues] = bP3Lsoftmax(trainInputs, trainTargets, learningRate, iterations, testInputs, testTargets, testLabels)
...
accValues = zeros(max((iterations/100),1), 1);
...
while( mseIter > 0.0005 && iters < iterations + 1)
    ...
    x = mod(iters,100);
    if(x == 0)
        accValues(iters/100,1) = determineAccuracy((validTest3LayReLU(testInputs, testTargets, W1, b1, W2, b2, W3, b3)), testLabels)
    end
end
